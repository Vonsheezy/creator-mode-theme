...
add_action( 'admin_menu', 'vonsheezy_elementor_settings_page' );
add_action( 'init', 'vonsheezy_elementor_tweak_settings', 0 );
...
function vonsheezy_elementor_settings_page() {
	...
	add_action( 'load-' . $menu_hook, function() {
		add_action( 'admin_enqueue_scripts', 'vonsheezy_elementor_settings_page_scripts', 10 );
	} );
 ...
 */
function vonsheezy_elementor_register_settings( $settings_group, $settings ) {
 ...
 */
function vonsheezy_elementor_do_tweak( $setting, $tweak_callback ) {
 ...
 * Render theme tweaks.
 */
function vonsheezy_elementor_render_tweaks( $settings_group, $settings ) {

	vonsheezy_elementor_do_tweak( $settings_group . $settings['DESCRIPTION_META_TAG'], function() {
		remove_action( 'wp_head', 'vonsheezy_elementor_add_description_meta_tag' );
	} );

	vonsheezy_elementor_do_tweak( $settings_group . $settings['SKIP_LINK'], function() {
		add_filter( 'vonsheezy_elementor_enable_skip_link', '__return_false' );
	} );

	vonsheezy_elementor_do_tweak( $settings_group . $settings['HEADER_FOOTER'], function() {
		add_filter( 'vonsheezy_elementor_header_footer', '__return_false' );
	} );

	vonsheezy_elementor_do_tweak( $settings_group . $settings['PAGE_TITLE'], function() {
		add_filter( 'vonsheezy_elementor_page_title', '__return_false' );
	} );

	vonsheezy_elementor_do_tweak( $settings_group . $settings['HELLO_STYLE'], function() {
		add_filter( 'vonsheezy_elementor_enqueue_style', '__return_false' );
	} );

	vonsheezy_elementor_do_tweak( $settings_group . $settings['HELLO_THEME'], function() {
